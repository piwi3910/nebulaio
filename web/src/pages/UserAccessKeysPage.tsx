...
export function UserAccessKeysPage() {
  ...
  const { data: user, isLoading: userLoading } = useQuery({
    ...
    queryFn: () => adminApi.getUser(userId!).then((res) => res.data as User),
    enabled: !!userId,
  });

  const { data: keys, isLoading: keysLoading } = useQuery({
    queryKey: ['user-access-keys', userId],
    queryFn: () => adminApi.listAccessKeys(userId!).then((res) => res.data),
    enabled: !!userId,
  });

  const createMutation = useMutation({
    mutationFn: (description: string) => adminApi.createAccessKey(userId!, description),
    onSuccess: (response) => {
      queryClient.invalidateQueries({ queryKey: ['user-access-keys', userId] });
      setCreateModalOpen(false);
      form.reset();
      setNewKey(response.data);
      notifications.show({
        title: 'Access key created',
        message: 'Make sure to save the secret key - it will only be shown once!',
        color: 'green',
      });
    },
    onError: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (accessKeyId: string) => adminApi.deleteAccessKey(accessKeyId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['user-access-keys', userId] });
  ...
  const isLoading = userLoading || keysLoading;

  return (
    <div>
      <Group mb="lg">
        <ActionIcon variant="subtle" size="lg" onClick={() => navigate('/users')}>
          <IconArrowLeft size={20} />
        </ActionIcon>
        <div>
          <Title order={2}>Access Keys</Title>
          <Text c="dimmed" size="sm">
            Manage access keys for {user?.username || 'user'}
          </Text>
        </div>
      </Group>
      ...
      <Paper withBorder>
        {isLoading ? (
          ...
          <Text c="dimmed" ta="center" p="xl">
            No access keys found for this user.
          </Text>
        )}
      </Paper>
