...
type PresignParams struct {
	...
	Expiration  time.Duration     // URL expiration duration (max 7 days)
	AccessKeyID string            // AWS access key ID
	SecretKey   string            // AWS secret access key
	Region      string            // AWS region (e.g., us-east-1)
	Headers     map[string]string // Optional headers to sign
	Endpoint    string            // Optional custom endpoint (e.g., http://localhost:9000)
	QueryParams map[string]string // Additional query parameters to include
}
